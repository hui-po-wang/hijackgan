...
def preprocess_image(image):
    img = image.transpose(0, 3, 1, 2)/255.
    img = (img - 0.5) / 0.5
    img = torch.tensor(img).float()
    return img


def compute_vgg_feat(image, vgg_model, is_rescale=True):
    img_tensor = preprocess_image(image).cuda()
    if is_rescale:
        rescale = torch.nn.Upsample(256, mode='bilinear')
        img_tensor = rescale(img_tensor)
    
    feat = vgg_model(img_tensor)
    out = []
    for kk in range(len(feat)):
        out.append(normalize_tensor(feat[kk]).detach().cpu().numpy())
    return out
...
def main():
  ...
  vgg_feats = defaultdict(list)
  pbar = tqdm(total=total_num, leave=False)
  vgg = lpips.pretrained_networks.vgg16().cuda() 
  for latent_codes_batch in model.get_batch_inputs(latent_codes):
    ...
    for key, val in outputs.items():
      if key == 'image':
        '''
        feats = compute_vgg_feat(val, vgg)
        for kk in range(len(feats)):
            vgg_feats[kk].append(feats[kk])
        '''    
        for image in val:
    ...
    if pbar.n % 1000 == 0 or pbar.n == total_num:
      ...
      # save vgg feats in the output directory in the pickle format:
      # {0:[feats], 1:[feats], ...}
      #with open(os.path.join(args.output_dir, 'vgg_feats.pkl'), 'wb') as f:
      #  pickle.dump(vgg_feats, f)
